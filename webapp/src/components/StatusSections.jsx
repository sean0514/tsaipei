...
export default function StatusSections({ statuses, tagMap, rows, statusKey = 'status', headerCells, renderRow, colSpan, emptyText = '目前沒有此狀態的紀錄。' }) {
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: 18 }}>
      {statuses.map((status) => {
        const items = rows.filter((r) => r[statusKey] === status);
        return (
